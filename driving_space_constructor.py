...
#from zzz_planning_decision_lane_models.Grid_Map import grid_map
...
class DrivingSpaceConstructor:
    def __init__(self, lane_dist_thres=5):
        ...
        self._grid_map_lock = Lock()
        self._grid_map_buffer = None
        self._grid_map_markerarray = None

        self._grid_map_obs_lock = Lock()
        self._grid_map_obs_buffer = None
        self._grid_map_obs_markerarray = None
    ...
    def receive_grid_map(self, gridmap):
        assert type(gridmap) == GridMapMsgStamped
        with self._grid_map_lock:
            self._grid_map_buffer = gridmap.g_map
            print("gridmap received ==================================")

    
    def receive_grid_map_obs(self, gridmapobs):
        assert type(gridmapobs) == GridMapMsgStamped
        with self._grid_map_obs_lock:
            self._grid_map_obs_buffer = gridmapobs.g_map
            print("gridmapobs received ==================================")
    ...
    def update_driving_space (self):
        ...
        tstates.grid_map = copy.deepcopy(self._grid_map_buffer)
        tstates.grid_map_obs = copy.deepcopy(self._grid_map_obs_buffer)
        ...
        #11. gridmap(ljx)
        grid_map_color = ColorRGBA()
        grid_map_color.r = 0.2
        grid_map_color.g = 0.2
        grid_map_color.b = 0.8
        grid_map_color.a = 0.5

        grid_map_obs_color1 = ColorRGBA()
        grid_map_obs_color1.r = 0.2
        grid_map_obs_color1.g = 0.6
        grid_map_obs_color1.b = 0.2
        grid_map_obs_color1.a = 0.5

        grid_map_obs_color2 = ColorRGBA()
        grid_map_obs_color2.r = 0.2
        grid_map_obs_color2.g = 0.5
        grid_map_obs_color2.b = 0.8
        grid_map_obs_color2.a = 0.5
        
        self._grid_map_markerarray = MarkerArray()
        if tstates.grid_map:
            tempmarker = self.visualize_grid_map(tstates.grid_map,grid_map_color, 0.5)
            tempmarker.ns = "zzz/cognition"          
            self._grid_map_markerarray.markers.append(tempmarker)

        self._grid_map_obs_markerarray = MarkerArray()
        if tstates.grid_map_obs:
            tempmarker = self.visualize_grid_map(tstates.grid_map_obs,grid_map_obs_color1,0.3)
            tempmarker.ns = "zzz/cognition"          
            self._grid_map_obs_markerarray.markers.append(tempmarker)

        rospy.logdebug("Updated driving space")

        return True

    def visualize_grid_map(self,grid_map,color1,z_in_rviz):
        
        tempmarker = Marker() 
        tempmarker.header.frame_id = "map"
        tempmarker.header.stamp = rospy.Time.now()
        tempmarker.id = 1
        tempmarker.type = Marker.LINE_LIST
        tempmarker.action = Marker.ADD
        tempmarker.pose.position = grid_map.position
        tempmarker.pose.orientation = grid_map.quaternion
        tempmarker.scale.x = 0.15
        tempmarker.lifetime = rospy.Duration(0.5)
        
        resol = 2*math.pi / grid_map.NUMOFGRID
        Point0 = Point()
        Point0.x = 0.0
        Point0.y = 0.0
        z = z_in_rviz
        ...
        Point0.z = z
        for i in range(grid_map.NUMOFGRID):
            
            temp = rect(grid_map.distancelist[i],i*resol)
            tempPoint = Point()
            tempPoint.x = temp.real
            tempPoint.y = temp.imag
            tempPoint.z = z
            tempmarker.points.append(Point0)
            tempmarker.points.append(tempPoint)

            tempmarker.colors.append(color1)
            tempmarker.colors.append(color1)
